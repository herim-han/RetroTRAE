...
class MyModel (pl.LightningModule, ):
    ...
    def training_step(self, batch, batch_idx):
        ...
        #kl_loss = lambda mean_1, mean_2=0, var_1, var_2=1: torch.log((var_2 / var_1) ** 0.5) + (var_1 + (mean_1 - mean_2) ** 2) / (2 * var_2) - 0.5
...
class Encoder(pl.LightningModule):
    ...
    def forward(self, src, e_mask=None, prop=None):
        ...
        #properties = torch.tensor((prop-mw_mean)/mw_std).unsqueeze(-1).to(torch.float32)
        properties = ((prop-mw_mean)/mw_std).clone().detach().unsqueeze(-1).to(torch.float32)
        #print(src.size(), e_mask.size(), properties.size(), properties)
